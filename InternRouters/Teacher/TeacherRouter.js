...
class TeacherRouter {
    constructor() {
        this.create = router.post('/create',  async (req, res) => {
            let {phone, name, mail, level, sex} = req.body;
            let validatedData = true;
            let dataError = "";
            let hashedPassword = await argon.hash("16026363");
            if(!validatedData){
                res.send({'finalResult': false,  'error': dataError});
            }else{
                let data = {phone,name, mail, level, sex, hashedPassword};
                try {
