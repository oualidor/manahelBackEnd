...
ContractRouter.post('/create',  async (req, res) => {
    console.log('/////////////////////////')
    let {name} = req.body;
    ...
    if(!validatedData){
        ...
        let data = {name};
        try {
            await ClassContract.create(data);
...
ContractRouter.get('/getAll/:offset/:limit',  async (req, res) => {
    let {offset, limit} = req.params;
    limit = parseInt(limit);
    offset = parseInt(offset);
    ClassContract.findAndCountAll({
        offset: offset,
        limit: limit,
    })
        .then(Classes =>
